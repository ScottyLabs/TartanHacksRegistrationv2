 ...
 * @param  {String}   name     Name of the proposed team
 * @param  {Function} callback args(err, users)
 */
UserController.createTeam = async function (id, name, res) {
  if (!name) {
    return callback({
  ...
  // TODO: check that user doesn't have team yet
  ...
  if (team) {
    return callback({
      message: "That team already exists"
    });
  } else {
    team = await new Team({
      name: name, 
...
UserController.sendResumeToDrive = Drive.sendResumeToDrive;

UserController.updateResumeOnDrive = Drive.updateResumeOnDrive;
